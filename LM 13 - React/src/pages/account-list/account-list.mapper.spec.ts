...
describe("pages/account-list/api/account-list.mapper test", () => {
  describe("mapAccountListFormApiToVm", () => {
    it("Should return empty array when it feeds empty array", () => {
      // Arrange
      const accountList: apiModel.Account[] = [];
      // Act
      const result = mapAccountListFormApiToVm(accountList);
      // Assert
      expect(result).toEqual([]);
    });

    it("Should return the same array but usign VM model structure", () => {
      // Arrange
      const accountList: apiModel.Account[] = [
        {
          id: "1",
